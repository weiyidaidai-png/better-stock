...
一个基于Python和Flask的A股股票筛选工具，使用tushare数据获取股票行情，计算长期均值和短期均值的差异，筛选出差异明显的股票，并提供Web界面展示结果和价格走势。

## 功能特点

- 📊 **数据获取**：获取全部A股股票列表和前复权日线行情
- 📈 **均值计算**：支持自定义长期均值周期和短期5日均线
- 🔍 **智能筛选**：按差异百分比阈值筛选股票，结果差异从大到小排序
...
系统支持申万一级行业分类（28类）的双重筛选机制：

#### （1）筛选前行业过滤
- 在股票数据加载阶段，可以选择特定行业进行预筛选
- 若指定了行业（如["电子", "电力设备"]），则仅加载这些行业的股票进行后续分析
- 支持空选择（即不限制行业，加载全部股票）

#### （2）筛选后行业过滤
- 在均值差异筛选结果生成后，可以基于结果内的行业进行二次过滤
- 系统会自动提取结果中存在的行业列表供选择
- 支持动态调整筛选条件，实时查看过滤结果
...
1. **数据获取与预筛选**：
   - 获取A股股票列表
   - （可选）根据用户选择的行业进行预筛选，仅保留指定行业的股票
   - 获取对应的前复权日线行情

2. **均值计算**：
   - 长期均值：最近N个交易日的收盘价平均值（N为长期周期参数）
   - 短期均值：最新的M日均线值（M为短期周期参数）

3. **差异计算**：`(短期均值 - 长期均值) / 长期均值 * 100%`

4. **初步筛选**：保留差异绝对值大于阈值的股票，按差异绝对值从大到小排序
...
### v2.0.0
- 🆕 新增申万一级行业分类（28类）筛选功能
- 🎯 支持"筛选前"行业预过滤，减少分析数据量
- 🔍 支持"筛选后"行业二次过滤，精细化筛选结果
- 📊 结果表格新增行业字段，方便查看股票所属行业
- 🌐 Web界面新增行业选择组件，支持多选和批量操作
- 🔄 优化筛选逻辑，支持动态调整行业筛选条件
